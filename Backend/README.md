...
프론트엔드 메인 페이지 및 위반 기록 페이지에서 사용할 위반 목록을 최신순으로 반환합니다. 위반 유형을 지정하면 해당 유형의 데이터만 필터링하여 제공합니다.

  * **URL:** `GET /api/violations`
  * **Query Parameter:**
      * `type` (선택): 조회할 위반 유형 (예: `헬멧 미착용`, `인도 주행`). 지정하지 않거나 `전체`로 요청 시 모든 내역 반환.
      * `limit` (선택): 가져올 데이터 개수 (기본값: 10)
      * 예시: `GET /api/violations?type=헬멧 미착용&limit=50`
  * **Response:**
    ```json
    [
      {
...
프론트엔드 통계 카드에 표시될 위반 건수를 반환합니다. 날짜 파라미터가 없을 경우 전체 누적 통계를, 날짜가 입력될 경우 해당 기간 내의 통계를 계산하여 반환합니다.

  * **URL:** `GET /api/stats`
  * **Query Parameter:**
      * `startDate` (선택): 통계 시작일 (형식: `YYYY-MM-DD`)
      * `endDate` (선택): 통계 종료일 (형식: `YYYY-MM-DD`)
      * 예시: `GET /api/stats?startDate=2026-03-01&endDate=2026-03-31` 
  * **Response:**
    ```json
    {
      "total": 12,
