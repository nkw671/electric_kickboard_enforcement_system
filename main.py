...
class ZoneDrawer:
    ...
    #             반투명 채우기와 외곽선으로 프레임에 그린다. (기존 코드 재사용)
    ...
    # 함수 이름 : _draw_dashed()
    # 기능      : 두 점 사이를 일정 간격의 점선으로 그린다. (기존 코드 재사용)
    ...
    # 함수 이름 : in_zone()
    # 기능      : 바운딩 박스의 하단 중심(발 위치)이 특정 Zone 내부인지 확인한다. (기존 코드 재사용)
    ...
    # 함수 이름 : save()
    # 기능      : 현재 zones 목록을 JSON 파일로 저장한다. (기존 코드 재사용)
    ...
    # 함수 이름 : load()
    # 기능      : JSON 파일에서 Zone 목록을 불러와 zones 에 저장한다. (기존 코드 재사용)
...
def run_detection():
    ...
    while True:
        ...
        # YOLO 로 객체를 추적한다. (기존 코드 재사용)
        results = model.track(frame, persist=True, conf=CONF, verbose=False, tracker = "bytetrack.yaml", vid_stride=2)

        drawer.draw_zones(frame)                # Zone 오버레이 렌더링 (기존 코드 재사용)

        # 감지된 객체가 있을 때만 처리한다. (기존 코드 재사용)
...
# 함수 이름 : set_zones()
# 기능      : React 캔버스에서 그린 Zone 목록을 받아 저장한다.
# 파라미터  : body = {"zones": [{"name": str, "pts": [[x,y],...], "color": [B,G,R]}, ...]}
# 반환값    : {"saved": Zone 개수}
@app.post("/zones")
async def set_zones(body: dict):
    drawer = app.state.drawer
    drawer.set_zones(body.get("zones", []))     # ZoneDrawer.set_zones() 재사용
    return {"saved": len(drawer.zones)}
...
async def delete_zones():
    drawer = app.state.drawer
    drawer.zones = []
    drawer.save(ZONE_FILE)                      # ZoneDrawer.save() 재사용
    return {"cleared": True}
...
if __name__ == "__main__":
    ...
    # 감지 루프를 별도 스레드에서 실행한다. (GUI 없이 백그라운드 동작)
